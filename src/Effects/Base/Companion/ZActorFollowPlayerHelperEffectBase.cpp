...
SFollowPlayerHelperBinding ZActorFollowPlayerHelperEffectBase::GetFollowHelperFor(
    const TEntityRef<ZActor> p_rActor
) const
{
    if (auto* s_pBlueprint = Utils::GetEntityBlueprintFactoryFor(p_rActor.m_entityRef))
    {
        if (const auto s_nIdx = s_pBlueprint->GetSubEntityIndex(0xcafee66c8c3ff219 /* _ChaosMod_FollowPlayerHelper */);
            s_nIdx != -1)
        {
