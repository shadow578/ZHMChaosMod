...
void ZSpawnRepositoryItemEffectBase::PushRepositoryProp(const ZRepositoryID& p_RepositoryId)
{
    ...
    m_aRepositoryProps.push_back({.m_RepositoryID = p_RepositoryId, .m_sCommonName = s_sCommonName, .m_sDisplayName = s_sDisplayName});
}
...
bool ZSpawnRepositoryItemEffectBase::SpawnRepositoryPropAt(const SRepositoryPropInfo& p_RepositoryProp, const float4 s_vPosition, const ZItemSpawner::EPhysicsMode p_ePhysicsMode)
{
    ...
    Logger::Debug(TAG "spawned item '{}' [{}] ({}).", p_RepositoryProp.m_sDisplayName, p_RepositoryProp.m_RepositoryID.ToString().c_str(), p_RepositoryProp.m_sCommonName);
    return true;
}
